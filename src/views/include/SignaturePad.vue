<template>
  <div
    class="relative w-full h-[184px] md:h-[200px] bg-[#FFFBE2] border border-l-lv4 rounded-sm overflow-hidden"
  >
    <Vue3Signature
      ref="signature"
    ...
    <div class="flex absolute top-0 right-0 gap-0.5 p-1 z-1 btn-group-signature">
      <Button
        ...
        label="이전"
        variant="text"
        class="[&>span:first-child]:text-t-lv1"
        @click="undo"
      />
      <Button
        size="small"
        ...
        label="다시 그리기"
        variant="text"
        class="[&>span:first-child]:text-red-500"
        @click="clear"
      />
...
import { useEstiAndConSetStore, useMateStore } from '@/store'
